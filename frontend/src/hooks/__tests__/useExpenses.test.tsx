...
describe('useCloneExpensesToNextMonth', () => {
  ...
  it('should handle year rollover when cloning from December', async () => {
    const mockResponse = {
      message: 'Successfully cloned 2 expenses to January 2025',
      cloned_count: 2,
      next_month_id: 13,
      next_month_name: 'January 2025',
    };
    ...
    result.current.mutate(12); // December month ID

    await waitFor(() => expect(result.current.isSuccess).toBe(true));

    expect(result.current.data).toEqual(mockResponse);
    expect(result.current.data?.next_month_name).toBe('January 2025');
  });

  it('should handle cloning with zero expenses', async () => {
    const mockResponse = {
      message: 'Successfully cloned 0 expenses to December 2024',
      cloned_count: 0,
      next_month_id: 2,
      next_month_name: 'December 2024',
    };
    ...
    expect(result.current.data?.cloned_count).toBe(0);
  });
});
