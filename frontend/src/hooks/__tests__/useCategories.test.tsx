...
describe('useCategorySummary', () => {
  ...
  it('should fetch category summary successfully', async () => {
    ...
    const { result } = renderHook(() => useCategorySummary({ month_id: 1 }), {
      wrapper: createWrapper(),
    });
    ...
    expect(categoriesApi.getSummary).toHaveBeenCalledWith({ month_id: 1 });
  });

  it('should fetch category summary without filters', async () => {
    const mockSummary: CategorySummary[] = [];
