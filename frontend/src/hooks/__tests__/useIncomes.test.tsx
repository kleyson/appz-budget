...
describe('useIncomes', () => {
  ...
  it('should fetch incomes with filters', async () => {
    const mockIncomes: Income[] = [
      {
        id: 1,
        income_type_id: 1,
    ...
    const { result } = renderHook(() => useIncomes({ period: 'Period 1', month_id: 1 }), {
      wrapper: createWrapper(),
    });
    ...
    expect(result.current.data).toEqual(mockIncomes);
    expect(incomesApi.getAll).toHaveBeenCalledWith({
      period: 'Period 1',
      month_id: 1,
    });
  });
...
describe('useIncome', () => {
  ...
  it('should fetch income by ID successfully', async () => {
  ...
  it('should skip fetch when ID is 0', async () => {
