...
For development or manual setup, see [docs/PROJECT.MD](docs/PROJECT.MD) for detailed instructions.
...
3. Configure the API URL in the app:

   - On first launch, the app will prompt you to configure the API endpoint
   - Go to Settings → API Configuration
   - Enter your backend URL and API key
   - Default demo server: `https://budget.appz.wtf` with API key `your-secret-api-key-change-this`
...
make verify     # Run all linting, formatting, type checks, and tests
```

### Development

```bash
make dev              # Start backend + frontend in parallel (development mode)
make backend          # Start backend only (auto-runs migrations)
make frontend         # Start frontend only
make mobile-dev       # Start mobile dev server (Expo)
...
```bash
make verify           # Run all lint, format, type checks, and tests (comprehensive)

# Backend
make lint-backend     # Lint backend code (ruff)
make lint-backend-fix # Lint and auto-fix backend code
make format-backend   # Format backend code (black + ruff)
make format-backend-check # Check backend formatting

# Frontend
make lint-frontend    # Lint frontend code (ESLint)
make lint-frontend-fix # Lint and auto-fix frontend code
make format-frontend  # Format frontend code (Prettier)
make format-frontend-check # Check frontend formatting
make type-check-frontend # Type check frontend (TypeScript)

# Mobile
make lint-mobile      # Lint mobile code
make type-check-mobile # Type check mobile code (TypeScript)

# TUI
make tui-lint         # Lint TUI code (cargo clippy)
make tui-format       # Format TUI code (cargo fmt)
make tui-format-check # Check TUI formatting
```
...
make tui-build-release # Build TUI (release, optimized)
```

### TUI Cross-Compilation

```bash
make tui-build-linux-x64    # Build for Linux x86_64
make tui-build-linux-arm64  # Build for Linux ARM64
make tui-build-windows      # Build for Windows x86_64
make tui-build-macos-x64    # Build for macOS x86_64 (requires macOS)
make tui-build-macos-arm64  # Build for macOS ARM64 (requires macOS)
make tui-build-all          # Build for all platforms (Linux + Windows)
```
...
make version VERSION="1.0.1"  # Update version and create git tag
```
