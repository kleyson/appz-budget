...
bump-build: ## Bump mobile build number (iOS buildNumber + Android versionCode)
	@echo "Bumping mobile build numbers..."
	@# Get current build numbers
	@CURRENT_BUILD=$$(grep -o '"buildNumber": "[0-9]*"' mobile/app.json | grep -o '[0-9]*'); \
	CURRENT_CODE=$$(grep -o '"versionCode": [0-9]*' mobile/app.json | grep -o '[0-9]*'); \
	NEW_BUILD=$$((CURRENT_BUILD + 1)); \
	NEW_CODE=$$((CURRENT_CODE + 1)); \
	echo "iOS buildNumber: $$CURRENT_BUILD -> $$NEW_BUILD"; \
	echo "Android versionCode: $$CURRENT_CODE -> $$NEW_CODE"; \
	sed -i '' "s/\"buildNumber\": \"[0-9]*\"/\"buildNumber\": \"$$NEW_BUILD\"/" mobile/app.json; \
	sed -i '' "s/\"versionCode\": [0-9]*/\"versionCode\": $$NEW_CODE/" mobile/app.json; \
	echo "✅ Updated mobile/app.json"; \
	git add mobile/app.json; \
	git commit -m "Bump mobile build to $$NEW_BUILD"; \
	echo "✅ Committed build number bump"

version: ## Update app version and create git tag (usage: make version VERSION="1.0.1")
	@if [ -z "$(VERSION)" ]; then \
	...
	@echo "Syncing version to mobile/src/utils/version.ts..."
	...
	@echo "};" >> mobile/src/utils/version.ts
	@echo "✅ Synced version to mobile/src/utils/version.ts"
	@echo "Updating mobile/app.json version..."
	@sed -i '' 's/"version": "[^"]*"/"version": "$(VERSION)"/' mobile/app.json
	@echo "✅ Updated mobile/app.json version to $(VERSION)"
	@echo ""
	@echo "Creating git commit and tag..."
	@git add VERSION mobile/src/utils/version.ts mobile/app.json
