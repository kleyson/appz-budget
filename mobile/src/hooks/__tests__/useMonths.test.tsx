...
describe('useCloseMonth', () => {
  ...
  it('should close month successfully', async () => {
    const mockResponse: MonthCloseResponse = {
      id: 1,
      name: 'November 2024',
      is_closed: true,
    ...
    vi.mocked(monthsApi.close).mockResolvedValue({
      data: mockResponse,
    } as any);

    const { result } = renderHook(() => useCloseMonth(), {
      wrapper: createWrapper(),
    });
    ...
    expect(monthsApi.close).toHaveBeenCalledWith(1);
  });

  it('should handle error when closing month', async () => {
    const error = new Error('Failed to close');
    vi.mocked(monthsApi.close).mockRejectedValue(error);
...
describe('useOpenMonth', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('should open month successfully', async () => {
    const mockResponse: MonthCloseResponse = {
      id: 1,
      name: 'November 2024',
      is_closed: false,
      message: 'Month opened successfully',
    };

    vi.mocked(monthsApi.open).mockResolvedValue({
      data: mockResponse,
    } as any);

    const { result } = renderHook(() => useOpenMonth(), {
      wrapper: createWrapper(),
    });
    ...
    expect(result.current.data).toEqual(mockResponse);
    expect(monthsApi.open).toHaveBeenCalledWith(1);
  });

  it('should handle error when opening month', async () => {
    const error = new Error('Failed to open');
    vi.mocked(monthsApi.open).mockRejectedValue(error);
